...
const getResponse = (url, currWallet, httpMethod, funded) => {
  let response = {}
  ...
  if (httpMethod === 'GET') {
    if (path.match(/\/v2\/wallet\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\?refresh=true&amount=[0-9]+&altcurrency=BAT/i)) {
      return JSON.stringify(reconcileResponses['reconcileWalletQueryResp'][funded || 'sufficientFunds'])
...
const request = (options, currWallet, callback, httpMethod, funded) => {
  let params
  const responseBody = getResponse(options.url, currWallet, httpMethod, funded)
...
const roundtrip = (params, options, callback, currWallet, funded) => {
  ...
  request(options, currWallet, requestCallbackFn, params.method, funded)
}
